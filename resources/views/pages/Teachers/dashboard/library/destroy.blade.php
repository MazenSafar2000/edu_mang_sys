        ...
        <form action="{{ route('library.destroy', $book->id) }}" method="post">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 style="font-family: 'Cairo', sans-serif;" class="modal-title" id="exampleModalLabel">{{ __('main_trans.delete_book')}}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <div class="modal-body">
                    <p>{{ trans('main_trans.Warning_Book') }} <span class="text-danger">{{ $book->title }}</span></p>
                    <input type="hidden" name="file_name" value="{{ $book->file_name }}">
                    <input type="hidden" name="teacher_name" value="{{Auth::user()->getTranslation('Name', 'en')}}_{{Auth::user()->getTranslation('Name', 'en')}}">
                </div>
